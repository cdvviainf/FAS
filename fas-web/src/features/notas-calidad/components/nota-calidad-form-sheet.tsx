...
export function NotaCalidadFormSheet({ item, open, onOpenChange }: NotaCalidadFormSheetProps) {
  const isEdit = !!item
  ...
  const { data: especies } = useQuery({
    queryKey: ['especies-options-nota-calidad'],
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (isEdit) return notasCalidadService.update(item!.id, payload)
      return notasCalidadService.create({ ...payload, codigo: codigo.trim() })
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['notas-calidad'] })
      toast.success(isEdit ? 'Nota de Calidad actualizada' : 'Nota de Calidad creada')
      onOpenChange(false)
    },
    onError: (e: Error) => toast.error(e.message || 'Error al guardar la Nota de Calidad'),
  })
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className='flex w-full flex-col sm:max-w-lg'>
        <SheetHeader>
          <SheetTitle>{isEdit ? `Editar Nota de Calidad ${item?.codigo}` : 'Nueva Nota de Calidad'}</SheetTitle>
          <SheetDescription>Catálogo de notas (ej. A, B, C, D) y las especies para las que es válida.</SheetDescription>
        </SheetHeader>
        ...
        <SheetFooter>
          ...
          <Button onClick={handleSubmit} isLoading={mutation.isPending}>
            <Icons.check className='mr-1 h-4 w-4' /> {isEdit ? 'Guardar cambios' : 'Crear Nota de Calidad'}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
...
export function NotaCalidadFormSheetTrigger() {
  ...
  return (
    <>
      <Button onClick={() => setOpen(true)}>
        <Icons.add className='mr-2 h-4 w-4' /> Nueva Nota de Calidad
      </Button>
      <NotaCalidadFormSheet open={open} onOpenChange={setOpen} />
    </>
  )
}
