...
interface UnidadMedidaQuickCreateProps {
  onCreated: (item: MantenedorSimple) => void
}

function UnidadMedidaQuickDialog({
  open,
  onOpenChange,
  onCreated
}: {
  ...
  const mutations = createMantenedorMutations('unidades-medida')
  const { keys } = createMantenedorQueries('unidades-medida')

  const mutation = useMutation({
    ...mutations.create,
    onSuccess: (newItem) => {
      queryClient.invalidateQueries({ queryKey: keys.all })
      toast.success(`Unidad "${newItem.descripcion}" creada`)
      onCreated(newItem)
      onOpenChange(false)
      form.reset()
    },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-sm'>
        <DialogHeader>
          ...
          <DialogDescription>
            La unidad quedará disponible inmediatamente en el selector.
          </DialogDescription>
        </DialogHeader>

        <form.AppForm>
          <form.Form id='um-quick-form' className='space-y-3'>
            <FormTextField name='codigo' label='Código' required placeholder='Ej: KG' />
            <FormTextField name='descripcion' label='Descripción' required placeholder='Ej: Kilogramo' />
            <FormTextField name='descripcionExtranjera' label='Descripción extranjera' placeholder='Ej: Kilogram' />
          </form.Form>
        </form.AppForm>

        <DialogFooter>
          <Button type='button' variant='outline' onClick={() => onOpenChange(false)}>Cancelar</Button>
          <Button type='submit' form='um-quick-form' isLoading={mutation.isPending}>
