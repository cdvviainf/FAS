...
interface TipoParametroQuickCreateProps {
  onCreated: (item: MantenedorSimple) => void
}

function TipoParametroQuickDialog({
  open,
  onOpenChange,
  onCreated
}: {
  ...
  const mutations = createMantenedorMutations('tipos-parametro')
  const { keys } = createMantenedorQueries('tipos-parametro')

  const mutation = useMutation({
    ...mutations.create,
    onSuccess: (newItem) => {
      queryClient.invalidateQueries({ queryKey: keys.all })
      toast.success(`Tipo de Parámetro "${newItem.descripcion}" creado`)
      onCreated(newItem)
      onOpenChange(false)
      form.reset()
    },
    onError: () => toast.error('Error al crear el tipo de parámetro')
  })
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-sm'>
        <DialogHeader>
          <DialogTitle>Nuevo Tipo de Parámetro</DialogTitle>
          <DialogDescription>
        ...
        <form.AppForm>
          <form.Form id='tipo-parametro-quick-form' className='space-y-3'>
            <FormTextField name='codigo' label='Código' required placeholder='Ej: CALIDAD' />
            <FormTextField name='descripcion' label='Descripción' required placeholder='Ej: Parámetros de Calidad' />
            <FormTextField name='descripcionExtranjera' label='Descripción extranjera' placeholder='Ej: Quality Parameters' />
          </form.Form>
        </form.AppForm>
        ...
        <DialogFooter>
          ...
          <Button type='submit' form='tipo-parametro-quick-form' isLoading={mutation.isPending}>
  ...
  return (
    <>
      <Button
        ...
        title='Crear nuevo tipo de parámetro'
      >
