// CLI: carga (o valida) un Excel de Carga Masiva contra la BD. Uso:
//   tsx prisma/carga-maestros-cargar.ts <archivo.xlsx> [--commit] [--empresa=AGROSAN] [--reporte=salida.xlsx]
// Por defecto corre en dry-run (no escribe). Con --commit escribe reutilizando
// los services de cada módulo.
import { writeFile } from 'node:fs/promises'
import { prisma } from '../src/lib/prisma.js'
import { cargarMaestros } from '../src/modules/config/carga-maestros/carga-maestros.service.js'
...
async function main() {
  ...
  if (!archivo || archivo.startsWith('--')) {
    console.error('Uso: tsx prisma/carga-maestros-cargar.ts <archivo.xlsx> [--commit] [--empresa=AGROSAN] [--reporte=salida.xlsx]')
    process.exit(1)
  }
  const commit = process.argv.includes('--commit')
  const codigoEmpresa = arg('empresa') ?? 'AGROSAN'
  const salida = arg('reporte') ?? 'Reporte_Carga_Masiva.xlsx'
  ...
  console.log(`\n=== Carga Masiva ${commit ? '(COMMIT)' : '(dry-run)'} — empresa ${codigoEmpresa} ===`)
  const res = await cargarMaestros(archivo, { empresaId: empresa.id, dryRun: !commit })

  for (const [hoja, r] of Object.entries(res.resumen)) {
    const nErr = res.errores.filter((e) => e.hoja === hoja).length
    console.log(`  ${hoja.padEnd(16)} filas=${String(r.filas).padStart(4)}  creados=${String(r.creados).padStart(4)}  errores=${nErr}`)
  }
