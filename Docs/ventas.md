...
> **Implementado (2026-09-02).** `Pallet.embarqueId` (compras.md §4.5/§4.7) + pestaña "Seleccionar Pallets" del Embarque: reserva/desvincula pallets uno a uno o en bloque, validando R8 (solo catálogo — especie/variedad/categoría/artículo/calibre — **sin** tope de cantidad, decisión de negocio, Christian; §10 Q3 queda resuelta así, no pendiente). `Embarque.despachadoEn` es el único gatillo de bloqueo hoy: una vez seteado, ya no se puede desvincular un pallet (agregar sigue permitido — decisión de negocio, Christian).

> **Implementado (2026-09-22) — cierra EP-QA-003.** El despacho SÍ exige reconciliación contra Packing List (compras.md §9.3): la pestaña "Despachar" exige subir el Excel (Template de Carga tipo `PACKING_LIST`) y reconciliarlo en estado `OK` antes de habilitar "Confirmar Despacho" — ver `embarques.packing-list.motor.ts`/`EmbarquePackingList` en `compras.md` §9.3 para el detalle del motor y el modelo.
>
> **Anular Despacho (2026-09-22, decisión de negocio Christian).** `PATCH /embarques/:id/anular-despacho` — soft delete de la confirmación: `despachadoEn`/`despachadoPor` quedan como registro histórico de la última confirmación (no se limpian), se marca `despachoAnuladoEn`/`despachoAnuladoPor`. "Efectivamente despachado" = `despachadoEn != null && despachoAnuladoEn == null` (todos los gatillos que antes leían `despachadoEn` a secas — bloqueo de desvincular pallet, bloqueo de subir Packing List, `confirmarDespacho` — pasan a leer esta condición compuesta). Los pallets **no** se desvinculan al anular (decisión de negocio explícita: "no desmarcarlos, dejarlos seleccionados") — para devolverlos a Stock hay que desvincularlos a mano desde "Seleccionar Pallets", ahora posible de nuevo. Bloqueado si el Embarque tiene Reclamos activos (`eliminadoEn: null`) — incompatible con "esta fruta en realidad no salió". Un `confirmarDespacho` posterior pisa la confirmación anterior y limpia la anulación — no se conserva historial de ciclos completos del despacho, solo el vigente.
>
> **`InstructivoHijo` y `EmbarquePackingList` pasan a soft delete (2026-09-22, decisión de negocio Christian).** Ambos ganan `eliminadoEn`/`eliminadoPor` — al anular el despacho, o al sincronizar "Generar Instructivos" (Planta que dejó de aportar pallets) o al subir un Packing List nuevo, la fila anterior se soft-elimina en vez de borrarse de verdad, para conservar el historial de qué instructivo/reconciliación tuvo cada despacho (incluidos los anulados). Todas las consultas de listado/vigencia filtran `eliminadoEn: null`; `codigo` de `InstructivoHijo` sigue siendo único para siempre (nunca se reutiliza un número de secuencia). Como Prisma no soporta índices únicos parciales en el DSL, la unicidad real (`empresaId`+`embarqueId`[+`plantaId`]) es un índice único a nivel de BD solo entre filas activas (`WHERE "eliminadoEn" IS NULL`) — mismo patrón que `Recepcion.ordenCompraId` (compras.md §4.4).
>
> **Ampliación (2026-09-02, compras.md §4.8).** Desde que `Pallet.completo` existe, "Seleccionar Pallets" **solo** lista pallets con `completo = true` (`getPalletsDisponibles` filtra `completo: true` además de `embarqueId: null`); la reserva (`reservarPalletsEnEmbarque`) revalida `completo: true` server-side como defensa en profundidad. Un pallet incompleto sigue siendo stock disponible (visible en el reporte de Stock/Gestión de Pallets), simplemente no es elegible para Embarque hasta marcarse Completo.
...
> **Implementado (2026-09-21) — cierra EP-QA-004.** `InstructivoHijo` existe en el schema (empresaId, embarqueId, codigo, secuencia, plantaId, fechaCargaPlanta, stackingDesde, stackingHasta, observaciones, auditoría; `@@unique([empresaId, embarqueId, plantaId])` y `@@unique([empresaId, codigo])`). A diferencia del texto de R11 arriba, la generación **NO** es automática al reservar/desvincular pallets — es un **botón explícito** ("Generar Instructivos" en la pestaña del mismo nombre, decisión de negocio Christian): agrupa por `Pallet.recepcion.plantaId` entre los pallets ya reservados al Embarque, crea un hijo por Planta nueva y elimina los de Plantas que ya no tienen pallets reservados, sin tocar los campos ya editados de los que se mantienen (secuencia/código estables una vez asignados). `PATCH /embarques/:id/instructivos/:instructivoId` edita fechaCargaPlanta/observaciones por hijo (stacking se trasladó a `Embarque`, ver ampliación 2026-09-22 más abajo).
>
> **Campos nuevos en `Embarque` (2026-09-21, gap analysis contra un Instructivo de Embarque real del cliente — "AGA").** Compartidos por todo el Embarque, independientes de `reservaManual`, editables vía `PATCH /embarques/:id/datos-instructivo`: `puertoZarpeId` (FK `Puerto`), `voyageNumber`, `deposito`, `awbBl`, `cutoffDate`, `tipoBultos`, `agenteAduanaId` (Entidad tipo `AGENTE_ADUANA`), `embarcadorId` (Entidad tipo `COMPANIA_EMBARQUE`), `navieraId` (Entidad tipo `NAVIERA` — reemplaza el texto libre `navieraManual` que antes vivía en la reserva manual; mismo dato, ya no duplicado). Ninguno de estos campos es derivable del Cierre Comercial ni de Selección de Pallets (lo que sí lo era — ej. Planta — no se agregó de nuevo).
>
> **PDF (Motor de Documentos).** Tipo `instructivo-embarque` en `documentos.registry.ts`, sin control de copia (mismo criterio que Instructivo de Embalaje): un documento **por `InstructivoHijo`** (no por Embarque completo), con el detalle de carga (pallet/especie/variedad/categoría/calibre/artículo/productor/cajas) filtrado a los pallets de esa Planta. Formato alineado al resto de los reportes del motor (`GrupoCampos`/`TablaLineas`/`ui/print-css.ts`), no al layout de grilla del documento original del cliente.
>
> **Migración `navieraManual` → `navieraId` sin backfill — decisión de negocio (Christian, 2026-09-21, QA ronda 2, FAS-IE-QA-001).** La migración `20260921160000_instructivo_embarque` elimina la columna `navieraManual` (texto libre) sin conservar ni reconciliar su contenido contra `navieraId`. Esto es intencional: el módulo `Embarque` no tiene despliegue productivo todavía (`embarques` no tiene filas reales en ningún ambiente a la fecha) — no existe dato histórico que reconciliar. No aplica el criterio de "migración en fases" usado en otras tablas con datos reales (ver compras.md §4.10 para el caso contrario, con `pallet_lineas` sí poblada).
>
> **Ampliación (2026-09-22) — Fecha de Arribo, Stacking (N rangos), Observaciones generales, un solo bloque.** `Embarque` gana `fechaArribo` (fecha estimada/real a destino) y `observacionesInstructivo` (generales, todo el Instructivo — distintas de `InstructivoHijo.observaciones`, por Planta). El stacking se **traslada** desde `InstructivoHijo` a `Embarque` (decisión de negocio, Christian: es una propiedad del contenedor/nave, compartida por todas las plantas de retiro, no un hito propio de cada una) y pasa de un par de fechas a **N rangos** (`EmbarqueStackingRango[]` — un Embarque puede tener más de una ventana de stacking, ej. dos días distintos); el conjunto completo se reemplaza en cada guardado (delete-then-create), nunca se edita un rango individual. Migraciones sin backfill, mismo criterio que `navieraManual` arriba (sin filas reales que reconciliar). `Embarque.fechaRetiroPlantaManual` se **elimina** (decisión de negocio, Christian) — el campo "Retiro Planta" sale del formulario sin reemplazo. La pestaña "Generar Instructivos" pierde Stacking de su formulario/tabla por hijo — conserva solo `fechaCargaPlanta` y `observaciones` (por Planta). En el frontend, la pestaña "Solicitud de Reserva" deja de tener dos cards separadas ("Reserva Manual" / "Datos del Instructivo") — pasa a **un solo bloque** con los campos en pares lógicos (Booking/AWB-BL, Contenedor/Naviera, Nave/N° Viaje, Fecha Embarque/Fecha Arribo, Puerto Embarque/Depósito, Embarcador/Agente de Aduana, Tipo de Bulto/Cutoff) y **un solo botón "Guardar"** — siguen siendo dos `PATCH` distintos en el backend (`/datos-reserva` y `/datos-instructivo`), pero un único click dispara ambos. Los campos de booking (Booking/Contenedor/Nave/Fecha Embarque) se muestran editables solo en modo manual; en modo automático (AGL360 confirmado) se muestran de solo lectura con el valor de `SolicitudReserva`. La vista del detalle del Embarque se ensancha de `max-w-4xl` a `max-w-[70rem]` (+25%) para dar cabida a los campos nuevos.
...
| GET | `/embarques/:id/instructivos` | Hijos por punto de retiro ya generados (R11 — **implementado** 2026-09-21). |
| POST | `/embarques/:id/instructivos/generar` | Botón explícito: deriva/sincroniza los hijos desde los pallets reservados, agrupando por Planta. |
| PATCH | `/embarques/:id/instructivos/:instructivoId` | Edita fechaCargaPlanta/observaciones de un hijo (stacking vive en `Embarque`, ver §4.2). |
| PATCH | `/embarques/:id/datos-instructivo` | Campos compartidos del Instructivo (puerto de zarpe, voyage, depósito, AWB/BL, cutoff, tipo de bultos, agente de aduana, embarcador, naviera, fecha de arribo, N rangos de stacking, observaciones generales). |
