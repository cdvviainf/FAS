...
>
> **Ampliación (2026-09-22) — Fecha de Arribo, Stacking trasladado, Observaciones generales.** `Embarque` gana `fechaArribo` (fecha estimada/real a destino) y `observacionesInstructivo` (generales, todo el Instructivo — distintas de `InstructivoHijo.observaciones`, por Planta). `stackingDesde`/`stackingHasta` se **trasladan** desde `InstructivoHijo` a `Embarque` (decisión de negocio, Christian): la ventana de stacking es una propiedad del contenedor/nave, compartida por todas las plantas de retiro, no un hito propio de cada una — migración sin backfill, mismo criterio y motivo que `navieraManual` arriba (sin filas reales que reconciliar). Los 4 campos se editan junto al resto de "Datos del Instructivo" (`PATCH /embarques/:id/datos-instructivo`, pestaña "Solicitud de Reserva") y aparecen en el PDF. La pestaña "Generar Instructivos" pierde Stacking de su formulario/tabla por hijo — conserva solo `fechaCargaPlanta` y `observaciones` (por Planta). El frontend unifica el guardado de la pestaña "Solicitud de Reserva" en **un solo botón "Guardar"** para ambas cards (Reserva Manual + Datos del Instructivo) — siguen siendo dos `PATCH` distintos, pero un único click dispara ambos. La vista del detalle del Embarque se ensancha de `max-w-4xl` a `max-w-[70rem]` (+25%) para dar cabida a los campos nuevos.
...
| PATCH | `/embarques/:id/datos-instructivo` | Campos compartidos del Instructivo (puerto de zarpe, voyage, depósito, AWB/BL, cutoff, tipo de bultos, agente de aduana, embarcador, naviera, fecha de arribo, stacking desde/hasta, observaciones generales). |
