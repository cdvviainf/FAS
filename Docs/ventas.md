...
> **Implementado (2026-09-21) — cierra EP-QA-004.** `InstructivoHijo` existe en el schema (empresaId, embarqueId, codigo, secuencia, plantaId, fechaCargaPlanta, stackingDesde, stackingHasta, observaciones, auditoría; `@@unique([empresaId, embarqueId, plantaId])` y `@@unique([empresaId, codigo])`). A diferencia del texto de R11 arriba, la generación **NO** es automática al reservar/desvincular pallets — es un **botón explícito** ("Generar Instructivos" en la pestaña del mismo nombre, decisión de negocio Christian): agrupa por `Pallet.recepcion.plantaId` entre los pallets ya reservados al Embarque, crea un hijo por Planta nueva y elimina los de Plantas que ya no tienen pallets reservados, sin tocar los campos ya editados de los que se mantienen (secuencia/código estables una vez asignados). `PATCH /embarques/:id/instructivos/:instructivoId` edita fechaCargaPlanta/stacking/observaciones por hijo.
>
...
| PATCH | `/embarques/:id/instructivos/:instructivoId` | Edita fechaCargaPlanta/stacking/observaciones de un hijo. |
| PATCH | `/embarques/:id/datos-instructivo` | Campos compartidos del Instructivo (puerto de zarpe, voyage, depósito, AWB/BL, cutoff, tipo de bultos, agente de aduana, embarcador, naviera). |
