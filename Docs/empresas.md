...
- **`PrefijoCodigo`**: único pasa de `(modelo)` a `(empresaId, modelo)` — cada empresa configura su propio prefijo/dígitos por mantenedor. **Refinamiento (2026-08-13):** gana `tipoEmbarqueId` opcional, solo para `modelo='embarque'` (`ventas.md` R10) — un prefijo por Tipo de Embarque, no uno global. La unicidad queda partida en dos índices parciales: `(empresaId, modelo) WHERE tipoEmbarqueId IS NULL` para el resto de los modelos, `(empresaId, modelo, tipoEmbarqueId) WHERE tipoEmbarqueId IS NOT NULL` para embarque.
