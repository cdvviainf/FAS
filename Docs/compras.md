...
- `instructivos` (`RecepcionInstructivoEmbalaje`, N:M → InstructivoEmbalaje, **2026-09-01**) — solo en modo `PROCESO`; uno o más Instructivos (`APROBADA`/`CERRADA`) que respaldan los folios de esta Recepción, fijados al crear (ver supersesión abajo)
...
>
> **⚠️ Supersesión (2026-09-01) — modo Proceso, selección de Instructivo(s).** Reemplaza la decisión 2026-08-23 (folios de "cualquier Instructivo de la empresa", sin selección puntual). Ahora, al crear una Recepción `PROCESO`, el usuario **selecciona uno o más** Instructivos de Embalaje (`RecepcionInstructivoEmbalaje`, N:M, fijado al crear — no editable después) en estado `APROBADA` o `CERRADA` (cerrar un Instructivo no invalida sus folios `APROBADO` aún no recepcionados, así que sigue siendo una fuente válida). Cada N° de Pallet del Excel debe corresponder a un folio con `estado = APROBADO` perteneciente a **alguno de los Instructivos seleccionados** (no a cualquiera de la empresa); si el folio existe pero pertenece a un Instructivo no seleccionado, el error lo indica explícitamente en vez de reportarlo como inexistente. Además, las características de la fila (especie/variedad/categoría/artículo/calibre) deben coincidir con **alguna línea** (`InstructivoEmbalajeDetalle`) del Instructivo dueño del folio — no es un control exhaustivo línea a línea, basta con que exista una línea compatible (decisión de negocio, Christian: un Instructivo puede tener varias combinaciones de calibre/variedad/embalaje en distintas líneas, y Calidad ingresa folios contra el conjunto, no contra una línea específica; por eso tampoco existe un "saldo" o cupo por línea — `cantidadPallets`/`cajas` de la línea siguen siendo solo informativas). Motor de validación equivalente al modo OC (§7): compara todos los N° de Pallet contra folios Aprobados **antes** de insertar (todo o nada); al insertar cada Pallet, su folio pasa a `RECEPCIONADO` y guarda `palletId` — atómico y condicionado a `estado = APROBADO` (mismo patrón que el reclamo de folios de Etapa 1A) para que dos Recepciones concurrentes no reclamen el mismo folio dos veces. Como sí valida (a diferencia de Consignación), el estado final es `VALIDADA`, igual que modo Compra.
